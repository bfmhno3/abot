...
static int start_record_internal(snd_pcm_t *pcm) { return snd_pcm_start(pcm); }

static int stop_record_internal(snd_pcm_t *pcm) { return snd_pcm_drop(pcm); }
...
static int format_ms_to_alsa(const WAVEFORMATEX *wavfmt,
  ...
  tmp = snd_pcm_build_linear_format(wavfmt->wBitsPerSample,
                                    ...
                                    wavfmt->wBitsPerSample == 8 ? 1 : 0, 0);
  if (tmp == SND_PCM_FORMAT_UNKNOWN) return -EINVAL;
  *format = tmp;
  return 0;
}
...
static int set_hwparams(struct recorder *rec, const WAVEFORMATEX *wavfmt,
  ...
  if (rec->buffer_time == 0 || rec->period_time == 0) {
    ...
    assert(err >= 0);
    if (rec->buffer_time > 500000) rec->buffer_time = 500000;
...
static int xrun_recovery(snd_pcm_t *handle, int err) {
  if (err == -EPIPE) { /* over-run */
    if (show_xrun) printf("!!!!!!overrun happend!!!!!!");

    err = snd_pcm_prepare(handle);
    if (err < 0) {
      if (show_xrun)
        printf(
            "Can't recovery from overrun,"
            "prepare failed: %s\n",
            snd_strerror(err));
      return err;
    }
...
static void *record_thread_proc(void *para) {
  ...
  while (1) {
    ...
    if (rec->state == RECORD_STATE_CLOSING) break;

    if (rec->state < RECORD_STATE_RECORDING) usleep(100000);
...
static int prepare_rec_buffer(struct recorder *rec) {
  ...
  if (!rec->audiobuf) return -ENOMEM;
  return 0;
}
#endif

static int open_recorder_internal(struct recorder *rec, record_dev_id dev,
                                  WAVEFORMATEX *fmt) {
  int err = 0;
  ...
  err = create_record_thread((void *)rec, &rec->rec_thread);
  if (err) goto fail;

  return 0;
fail:
  if (rec->wavein_hdl) snd_pcm_close((snd_pcm_t *)rec->wavein_hdl);
  rec->wavein_hdl = NULL;
  free_rec_buffer(rec);
  return err;
}
...
static int get_pcm_device_cnt(snd_pcm_stream_t stream) {
  ...
  while (*n != NULL) {
    ...
    if (name && (io == NULL || strcmp(io, filter) == 0)) cnt++;
    if (io != NULL) free(io);
...
static int list_pcm(snd_pcm_stream_t stream, char **name_out, char **desc_out) {
  ...
  *name_out = calloc(sizeof(char *), (1 + cnt));
  if (*name_out == NULL) goto fail;
  *desc_out = calloc(sizeof(char *), (1 + cnt));
  if (*desc_out == NULL) goto fail;
...
int open_recorder(struct recorder *rec, record_dev_id dev, WAVEFORMATEX *fmt) {
  int ret = 0;
  if (!rec) return -RECORD_ERR_INVAL;
  if (rec->state >= RECORD_STATE_READY) return 0;

  ret = open_recorder_internal(rec, dev, fmt);
  if (ret == 0) rec->state = RECORD_STATE_READY;
  return 0;
}

void close_recorder(struct recorder *rec) {
  if (rec == NULL || rec->state < RECORD_STATE_READY) return;
  if (rec->state == RECORD_STATE_RECORDING) stop_record(rec);
...
int start_record(struct recorder *rec) {
  int ret;
  if (rec == NULL) return -RECORD_ERR_INVAL;
  if (rec->state < RECORD_STATE_READY) return -RECORD_ERR_NOT_READY;
  if (rec->state == RECORD_STATE_RECORDING) return 0;

  ret = start_record_internal((snd_pcm_t *)rec->wavein_hdl);
  if (ret == 0) rec->state = RECORD_STATE_RECORDING;
  return ret;
}
