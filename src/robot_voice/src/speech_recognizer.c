...
#define DEFAULT_SESSION_PARA                                                   \
  ...
  "= 16000, result_type = plain, result_encoding = gb2312"

#define DEFAULT_FORMAT \
  {WAVE_FORMAT_PCM, 1, 16000, 32000, 2, 16, sizeof(WAVEFORMATEX)}

/* internal state */
enum { SR_STATE_INIT, SR_STATE_STARTED };

#define SR_MALLOC malloc
#define SR_MFREE free
#define SR_MEMSET memset

static void Sleep(size_t ms) { usleep(ms * 1000); }

static void end_sr_on_error(struct speech_rec *sr, int errcode) {
  if (sr->aud_src == SR_MIC) stop_record(sr->recorder);

  if (sr->session_id) {
    if (sr->notif.on_speech_end) sr->notif.on_speech_end(errcode);
...
static void end_sr_on_vad(struct speech_rec *sr) {
  ...
  while (sr->rec_stat != MSP_REC_STATUS_COMPLETE) {
    ...
    if (rslt && sr->notif.on_result)
      sr->notif.on_result(rslt,
                          sr->rec_stat == MSP_REC_STATUS_COMPLETE ? 1 : 0);

    Sleep(100); /* for cpu occupy, should sleep here */
  }

  if (sr->session_id) {
    if (sr->notif.on_speech_end) sr->notif.on_speech_end(END_REASON_VAD_DETECT);
...
static void iat_cb(char *data, unsigned long len, void *user_para) {
  ...
  if (sr == NULL || sr->ep_stat >= MSP_EP_AFTER_SPEECH) return;
...
static int update_format_from_sessionparam(const char *session_para,
    ...
    if (s && *s) {
      ...
      if (s && *s) {
        ...
        wavefmt->nAvgBytesPerSec =
            wavefmt->nBlockAlign * wavefmt->nSamplesPerSec;
      }
    } else
      return -1;
  } else {
...
int sr_init(struct speech_rec *sr, const char *session_begin_params,
  ...
  return sr_init_ex(sr, session_begin_params, aud_src, get_default_input_dev(),
                    notify);
}
...
int sr_start_listening(struct speech_rec *sr) {
  ...
  session_id = QISRSessionBegin(
      NULL, sr->session_begin_params,
      &errcode);  // ��д����Ҫ�﷨����һ������ΪNULL
  ...
  if (sr->notif.on_speech_begin) sr->notif.on_speech_begin();
...
int sr_stop_listening(struct speech_rec *sr) {
  ...
  sr->state = SR_STATE_INIT;
  ret = QISRAudioWrite(sr->session_id, NULL, 0, MSP_AUDIO_SAMPLE_LAST,
                       &sr->ep_stat, &sr->rec_stat);
  if (ret != 0) {
...
int sr_write_audio_data(struct speech_rec *sr, char *data, unsigned int len) {
  ...
  ret = QISRAudioWrite(sr->session_id, data, len, sr->audio_status,
                       &sr->ep_stat, &sr->rec_stat);
  if (ret) {
    end_sr_on_error(sr, ret);
    return ret;
  ...
  if (MSP_EP_AFTER_SPEECH == sr->ep_stat) end_sr_on_vad(sr);
...
void sr_uninit(struct speech_rec *sr) {
  if (sr->recorder) {
    if (!is_record_stopped(sr->recorder)) stop_record(sr->recorder);
