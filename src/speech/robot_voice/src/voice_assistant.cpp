...
/* 默认wav音频头部数据 */
wave_pcm_hdr default_wav_hdr = {{'R', 'I', 'F', 'F'},
                                0,
                                {'W', 'A', 'V', 'E'},
                                {'f', 'm', 't', ' '},
                                16,
...
int text_to_speech(const char* src_text, const char* des_path,
  ...
  fwrite(&wav_hdr.data_size, sizeof(wav_hdr.data_size), 1,
         fp);  // 写入data_size的值
  fclose(fp);
  fp = NULL;
  /* 合成完毕 */
...
void voiceWordsCallback(const std_msgs::String::ConstPtr& msg) {
  ...
  const char* session_begin_params =
      "voice_name = xiaoyan, text_encoding = utf8, sample_rate = 16000, speed "
      "= 50, volume = 50, pitch = 50, rdn = 2";
  ...
  if (dataString.compare("你是谁？") == 0) {
    ...
    ts = time(NULL);
    ptm = localtime(&ts);
    std::string string = "现在时间" + to_string(ptm->tm_hour) + "点" +
                         to_string(ptm->tm_min) + "分";
...
int main(int argc, char* argv[]) {
  ...
  const char* login_params =
      "appid = 594a7b46, work_dir = .";  // 登录参数,appid与msc库绑定,请勿随意改动
  /*
   * rdn:           合成音频数字发音方式
   * volume:        合成音频的音量
   * pitch:         合成音频的音调
  ...
  printf(
      "## 语音合成（Text To "
      "Speech，TTS）技术能够自动将任意文字实时转换为连续的 ##\n");
  printf(
