...
class Navigation {
 public:
  ...
  void initROSModule() {
    ...
    status_sub = nh.subscribe<actionlib_msgs::GoalStatusArray>(
        "/move_base/status", 10, &Navigation::status_cb, this);
    nav_sub = nh.subscribe<std_msgs::String>("/robot_voice/nav_topic", 10,
                                             &Navigation::nav_cb, this);
    pos_sub = nh.subscribe<tf2_msgs::TFMessage>("/tf", 1000,
                                                &Navigation::pose_cb, this);
    exec_timer =
        nh.createTimer(ros::Duration(0.05), &Navigation::execCallback, this);
 ...
 private:
  ...
  void nav_cb(const std_msgs::String::ConstPtr &msg) { start_nav = true; }

  void pose_cb(const tf2_msgs::TFMessage::ConstPtr &msg) {
    tf::StampedTransform transform;
    try {
      pose_listener.lookupTransform("map", "base_link", ros::Time(0),
                                    transform);
  ...
  bool close_enough() {
    float abs_distance;
    abs_distance = sqrt(
        (base_pos[0] - current_target_X) * (base_pos[0] - current_target_X) +
        (base_pos[1] - current_target_Y) * (base_pos[1] - current_target_Y));
