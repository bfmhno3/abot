 ...
 public:
  ...
  void initROSModule() {  // 设置订阅器和发布器
    ...
    status_sub = nn.subscribe<actionlib_msgs::GoalStatusArray>(
        "/move_base/status", 10, &CamTrack::status_cb,
        this);  // move base status
    pos_sub =
        nn.subscribe<tf2_msgs::TFMessage>("/tf", 1000, &CamTrack::pos_cb, this);
    ar_sub =
        nn.subscribe<tf2_msgs::TFMessage>("/tf", 1000, &CamTrack::ar_cb, this);
    odom_sub =
        nn.subscribe<nav_msgs::Odometry>("/odom", 10, &CamTrack::odom_cb, this);
    ...
    exec_timer_ =
        nn.createTimer(ros::Duration(0.05), &CamTrack::execCallback, this);
  }
  float PID_Control_P;
  float Max_yaw_vel;
  float yaw_th;
 ...
 private:
  ...
  void ar_cb(const tf2_msgs::TFMessage::ConstPtr &msg) {
    ...
    if (msg->transforms[0].child_frame_id == "ar_marker_0") {
      try {
        ar_listener.lookupTransform("map", "ar_marker_0", ros::Time(0),
                                    transform);
      ...
      try {
        ar_listener.lookupTransform("map", "ar_marker_1", ros::Time(0),
                                    transform);
      } catch (tf::TransformException &ex) {
        ROS_INFO("Couldn't get transform");
        ROS_WARN("%s", ex.what());
      ...
      try {
        ar_listener.lookupTransform("map", "ar_marker_2", ros::Time(0),
                                    transform);
      } catch (tf::TransformException &ex) {
        ROS_INFO("Couldn't get transform");
        ROS_WARN("%s", ex.what());
      ...
      try {
        ar_listener.lookupTransform("map", "ar_marker_3", ros::Time(0),
                                    transform);
      } catch (tf::TransformException &ex) {
        ROS_INFO("Couldn't get transform");
        ROS_WARN("%s", ex.what());
