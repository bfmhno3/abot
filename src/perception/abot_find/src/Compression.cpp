...
std::vector<unsigned char> compressData(const cv::Mat& data) {
  ...
  if (!data.empty()) {
    ...
    int errCode = compress((Bytef*)bytes.data(), &destLen,
                           (const Bytef*)data.data, sourceLen);
    ...
    *((int*)&bytes[destLen + 2 * sizeof(int)]) = data.type();
    ...
    if (errCode == Z_MEM_ERROR) {
      ...
      UERROR(
          "Z_BUF_ERROR : The buffer dest was not large enough to hold the "
          "uncompressed data.");
    }
  }
  return bytes;
...
cv::Mat uncompressData(const unsigned char* bytes, unsigned long size) {
  ...
  if (bytes && size >= 3 * sizeof(int)) {
    ...
    int errCode = uncompress((Bytef*)data.data, &totalUncompressed,
                             (const Bytef*)bytes, uLong(size));
    ...
    if (errCode == Z_MEM_ERROR) {
      ...
      UERROR(
          "Z_DATA_ERROR : The compressed data (referenced by source) was "
          "corrupted.");
    }
  }
  return data;
