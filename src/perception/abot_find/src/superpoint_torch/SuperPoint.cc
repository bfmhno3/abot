...
void NMS(const std::vector<cv::KeyPoint>& ptsIn, const cv::Mat& conf,
         const cv::Mat& descriptorsIn, std::vector<cv::KeyPoint>& ptsOut,
         cv::Mat& descriptorsOut, int border, int dist_thresh, int img_width,
         int img_height);

SPDetector::SPDetector(const std::string& modelPath, float threshold, bool nms,
                       int minDistance, bool cuda)
    : threshold_(threshold),
      nms_(nms),
      minDistance_(minDistance),
      detected_(false) {
  UDEBUG("modelPath=%s thr=%f nms=%d cuda=%d", modelPath.c_str(), threshold,
         nms ? 1 : 0, cuda ? 1 : 0);
  if (modelPath.empty()) {
    return;
  }
  ...
  if (cuda && !torch::cuda::is_available()) {
    UWARN(
        "Cuda option is enabled but torch doesn't have cuda support on this "
        "platform, using CPU instead.");
  }
...
std::vector<cv::KeyPoint> SPDetector::detect(const cv::Mat& img) {
  ...
  if (model_) {
    ...
    auto x =
        torch::from_blob(img.data, {1, 1, img.rows, img.cols}, torch::kByte);
    ...
    if (nms_ && !keypoints_no_nms.empty()) {
      ...
      cv::Mat descEmpty;
      NMS(keypoints_no_nms, conf, descEmpty, keypoints, descEmpty, border,
...
cv::Mat SPDetector::compute(const std::vector<cv::KeyPoint>& keypoints) {
  if (!detected_) {
    UERROR(
        "SPDetector has been reset before extracting the descriptors! detect() "
        "should be called before compute().");
    return cv::Mat();
  }
  if (model_.get()) {
    ...
    grid[0][0].slice(1, 0, 1) =
        2.0 * fkpts.slice(1, 1, 2) / prob_.size(1) - 1;  // x
    grid[0][0].slice(1, 1, 2) =
        2.0 * fkpts.slice(1, 0, 1) / prob_.size(0) - 1;  // y

    auto desc = torch::grid_sampler(desc_, grid, 0, 0,
                                    true);  // [1, 256, 1, n_keypoints]
...
void NMS(const std::vector<cv::KeyPoint>& ptsIn, const cv::Mat& conf,
    ...
    if (grid.at<unsigned char>(vv, uu) == 100)  // If not yet suppressed.
    {
      for (int k = -dist_thresh; k < (dist_thresh + 1); k++) {
        for (int j = -dist_thresh; j < (dist_thresh + 1); j++) {
          ...
          if (confidence.at<float>(vv + k - dist_thresh,
                                   uu + j - dist_thresh) <= c) {
  ...
  grid =
      cv::Mat(grid, cv::Rect(dist_thresh, dist_thresh, img_width, img_height));
  ...
  if (!descriptorsIn.empty()) {
    ...
    for (size_t i = 0; i < select_indice.size(); i++) {
      for (int j = 0; j < 256; j++) {
        descriptorsOut.at<float>(i, j) =
            descriptorsIn.at<float>(select_indice[i], j);
      }
    }
  }
}
