...
const char* ULogger::levelName_[5] = {"DEBUG", " INFO", " WARN", "ERROR",
                                      "FATAL"};
...
class UConsoleLogger : public ULogger {
 ...
 private:
  virtual void _write(const char* msg, va_list arg) { vprintf(msg, arg); }
  virtual void _writeStr(const char* msg) { printf("%s", msg); }
};
...
void ULogger::setType(Type type, const std::string& fileName, bool append) {
  ...
  loggerMutex_.lock();
  {
    ...
    // type changed
             ...
             (type_ == kTypeFile && logFileName_.compare(fileName) != 0)) {
...
void ULogger::reset() {
  ...
  level_ = kInfo;  // By default, we show all info msgs + upper level (Warning,
                   // Error)
...
void ULogger::write(ULogger::Level level, const char* file, int line,
...
int ULogger::getTime(std::string& timeStr) {
  ...
  int result = sprintf_s(
      buf, bufSize, "%d-%s%d-%s%d %s%d:%s%d:%s%d", timeinfo.tm_year + 1900,
  ...
  int result = snprintf(
      buf, bufSize, "%d-%s%d-%s%d %s%d:%s%d:%s%d.%s%d", timeinfo.tm_year + 1900,
      (timeinfo.tm_mon + 1) < 10 ? "0" : "", timeinfo.tm_mon + 1,
      (timeinfo.tm_mday) < 10 ? "0" : "", timeinfo.tm_mday,
      (timeinfo.tm_hour) < 10 ? "0" : "", timeinfo.tm_hour,
      (timeinfo.tm_min) < 10 ? "0" : "", timeinfo.tm_min,
      (timeinfo.tm_sec) < 10 ? "0" : "", timeinfo.tm_sec,
      (rawtime.tv_usec / 1000) < 10    ? "00"
      : (rawtime.tv_usec / 1000) < 100 ? "0"
                                       : "",
