...
void UPlotItem::setData(const QPointF& data) { _data = data; }
...
UPlotCurve::UPlotCurve(const QString& name, QVector<UPlotItem*> data,
                       QObject* parent)
    : QObject(parent),
      _plot(0),
      _name(name),
...
UPlotCurve::UPlotCurve(const QString& name, const QVector<float>& x,
                       const QVector<float>& y, QObject* parent)
    : QObject(parent),
      _plot(0),
      _name(name),
      _xIncrement(1),
...
#if PRINT_DEBUG
  ULOGGER_DEBUG("curve=\"%s\" from plot=\"%s\"",
                this->objectName().toStdString().c_str(),
                plot ? plot->objectName().toStdString().c_str() : "");
#endif
  if (plot && _plot == plot) {
    _plot = 0;
    for (int i = 0; i < _items.size(); ++i) {
...
void UPlotCurve::addValue(const QString& value) {
  ...
  if (ok) {
    ...
    ULOGGER_ERROR("Value not valid, must be a number, received %s",
...
void UPlotCurve::removeItem(
    UPlotItem* item)  // ownership is transfered to the caller
{
  for (int i = 0; i < _items.size(); ++i) {
    if (_items.at(i) == item) {
      if (i != 0) {
...
void UPlotCurve::update(float scaleX, float scaleY, float offsetX,
                        float offsetY, float xDir, float yDir,
                        bool allDataKept) {
  // ULOGGER_DEBUG("scaleX=%f, scaleY=%f, offsetX=%f, offsetY=%f, xDir=%d,
  // yDir=%d, _plot->scene()->width()=%f, _plot->scene()->height=%f", scaleX,
  // scaleY, offsetX, offsetY, xDir,
  // yDir,_plot->scene()->width(),_plot->scene()->height()); make sure direction
  // values are 1 or -1
  ...
  for (int i = _items.size() - 1; i >= 0; --i) {
    if (i % 2 == 0) {
      ...
      if (hide) {
        if (allDataKept) {
          // if not visible, stop looping... all other items are normally
          // already hided
          if (!item->isVisible()) {
            break;
          }
        ...
        QPointF newPos(((xDir * item->data().x() + offsetX) * scaleX -
                        item->rect().width() / 2.0f),
                       ((yDir * item->data().y() + offsetY) * scaleY -
                        item->rect().width() / 2.0f));
        if (!item->isVisible()) {
      ...
      if (hide) {
        ...
        if ((line.x1() <= line.x2() &&
             line.x2() < 0 - ((line.x2() - line.x1()))) ||
            (line.x1() > line.x2() &&
             line.x2() > lineItem->scene()->sceneRect().width() +
                             ((line.x1() - line.x2())))) {
          hide = true;
        }
      }
...
void UPlotCurve::draw(QPainter* painter) {
  if (painter) {
    for (int i = _items.size() - 1; i >= 0 && _items.at(i)->isVisible();
         i -= 2) {
      // plotItem
      ...
      if (i - 1 >= 0) {
        ...
        // lineItem
        const QGraphicsLineItem* item =
            (const QGraphicsLineItem*)_items.at(i - 1);
      ...
      painter->drawEllipse(item->pos() + QPointF(item->rect().width() / 2,
                                                 item->rect().height() / 2),
                           (int)item->rect().width() / 2,
                           (int)item->rect().height() / 2);
    }
  }
}

int UPlotCurve::itemsSize() const { return _items.size(); }
...
void UPlotCurve::setXIncrement(float increment) { _xIncrement = increment; }

void UPlotCurve::setXStart(float val) { _xStart = val; }
...
UPlotCurveThreshold::UPlotCurveThreshold(const QString& name,
                                         ...
                                         QObject* parent)
    : UPlotCurve(name, parent), _orientation(orientation) {
...
void UPlotCurveThreshold::setThreshold(float threshold) {
  ...
  if (_items.size() == 3) {
    ...
    ULOGGER_ERROR(
        "A threshold must has only 3 items (1 PlotItem + 1 QGraphicsLineItem + "
        "1 PlotItem)");
  }
}
...
void UPlotCurveThreshold::update(float scaleX, float scaleY, float offsetX,
                                 float offsetY, float xDir, float yDir,
                                 bool allDataKept) {
  if (_items.size() == 3) {
    if (_plot) {
      ...
      if (_orientation == Qt::Horizontal) {
        ...
        item = (UPlotItem*)_items.at(2);
        item->setData(
            QPointF((_plot->sceneRect().width() / scaleX - offsetX) / xDir,
                    item->data().y()));
      } else {
        item = (UPlotItem*)_items.at(0);
        ...
        item->setData(
            QPointF(item->data().x(),
                    (_plot->sceneRect().height() / scaleY - offsetY) / yDir));
      }
      this->updateMinMax();
    }
  } else {
...
UPlotAxis::UPlotAxis(Qt::Orientation orientation, float min, float max,
                     QWidget* parent)
    : QWidget(parent),
      _orientation(orientation),
      _reversed(false),
...
void UPlotAxis::setAxis(float& min, float& max) {
  ...
  if (_orientation == Qt::Vertical) {
    ...
    borderMin =
        this->fontMetrics().width(QString::number(_min, 'g', _gradMaxDigits)) /
        2;
    borderMax =
        this->fontMetrics().width(QString::number(_max, 'g', _gradMaxDigits)) /
        2;
  }
  ...
  if (min != max) {
    ...
    min = float(minR) / mul;
    max = float(maxR) / mul;
    // ULOGGER_DEBUG("mul=%f, minR=%d, maxR=%d,countStep=%d", mul, minR, maxR,
    // countStep);
  }
  ...
  if (_orientation == Qt::Vertical) {
    ...
    for (int i = 0; i <= _count; i += 5) {
      QString n(QString::number(_min + (i / 5) * ((_max - _min) / (_count / 5)),
                                'g', _gradMaxDigits));
  ...
  if (_orientation == Qt::Vertical) {
    ...
    for (int i = 0; i <= _count; ++i) {
      if (i % 5 == 0) {
        ...
        QLabel n(
            QString::number(_min + (i / 5) * ((_max - _min) / (_count / 5)),
                            'g', _gradMaxDigits));
        painter.drawText(this->width() - (12 + n.sizeHint().width()),
                         n.sizeHint().height() / 2 - 2, n.text());
      } else {
...
UPlotLegendItem::UPlotLegendItem(const UPlotCurve* curve, QWidget* parent)
    : QPushButton(parent), _curve(curve) {
...
void UPlotLegendItem::contextMenuEvent(QContextMenuEvent* event) {
  ...
  if (action == _aChangeText) {
    bool ok;
    QString text =
        QInputDialog::getText(this, _aChangeText->text(), tr("Name :"),
                              QLineEdit::Normal, this->text(), &ok);
    if (ok && !text.isEmpty()) {
      this->setText(text);
    }
...
UPlotLegend::UPlotLegend(QWidget* parent) : QWidget(parent), _flat(true) {
  // menu
...
void UPlotLegend::addItem(const UPlotCurve* curve) {
  if (curve) {
    ...
    legendItem->setIcon(
        QIcon(this->createSymbol(curve->pen(), curve->brush())));
    legendItem->setIconSize(QSize(25, 20));
    connect(legendItem, SIGNAL(toggled(bool)), this,
            SLOT(redirectToggled(bool)));
    connect(legendItem, SIGNAL(legendItemRemoved(const UPlotCurve*)), this,
            SLOT(removeLegendItem(const UPlotCurve*)));
    ...
    // add to the legend
    ((QVBoxLayout*)this->layout())
        ->insertLayout(this->layout()->count() - 1, hLayout);
  }
}
...
UOrientableLabel::UOrientableLabel(const QString& text,
    ...
    : QLabel(text, parent), _orientation(orientation) {}

UOrientableLabel::~UOrientableLabel() {}

QSize UOrientableLabel::sizeHint() const {
  QSize size = QLabel::sizeHint();
  if (_orientation == Qt::Vertical) size.transpose();
  return size;
}
...
void UOrientableLabel::paintEvent(QPaintEvent* event) {
  ...
  p.drawText(r, this->alignment() | (this->wordWrap() ? Qt::TextWordWrap : 0),
             this->text());
}

UPlot::UPlot(QWidget* parent)
    : QWidget(parent), _maxVisibleItems(-1), _autoScreenCaptureFormat("png") {
  this->setupUi();
  this->createActions();
  this->createMenus();
...
void UPlot::setupUi() {
  ...
  connect(_legend, SIGNAL(legendItemToggled(const UPlotCurve*, bool)), this,
          SLOT(showCurve(const UPlotCurve*, bool)));
  connect(_legend, SIGNAL(legendItemRemoved(const UPlotCurve*)), this,
          SLOT(removeCurve(const UPlotCurve*)));
}
...
bool UPlot::addCurve(UPlotCurve* curve, bool ownershipTransferred) {
  if (curve) {
    ...
    if (!qobject_cast<UPlotCurveThreshold*>(curve)) {
      for (int i = _curves.size() - 1; i >= 0; --i) {
        if (!qobject_cast<UPlotCurveThreshold*>(_curves.at(i))) {
          disconnect(_curves.at(i), SIGNAL(dataChanged(const UPlotCurve*)),
    ...
    connect(curve, SIGNAL(dataChanged(const UPlotCurve*)), this,
            SLOT(updateAxis()));

    _legend->addItem(curve);

#if PRINT_DEBUG
    ULOGGER_DEBUG("Curve \"%s\" added to plot \"%s\"",
...
QStringList UPlot::curveNames() {
  QStringList names;
  for (QList<UPlotCurve*>::iterator iter = _curves.begin();
       iter != _curves.end(); ++iter) {
    if (*iter) {
...
void UPlot::replot(QPainter* painter) {
  if (_maxVisibleItems > 0) {
    ...
    for (QList<UPlotCurve*>::iterator i = _curves.begin(); i != _curves.end();
         ++i) {
    ...
    if (c && (maxItem - 1) / 2 + 1 > _maxVisibleItems) {
      _axisMaximums[0] =
          c->getItemData((c->itemsSize() - 1) - _maxVisibleItems * 2).x();
    }
  }
  ...
  // ULOGGER_DEBUG("x1=%f, x2=%f, y1=%f, y2=%f", _axisMaximums[0],
  // _axisMaximums[1], _axisMaximums[2], _axisMaximums[3]);

  QRectF newRect(0, 0, _graphicsViewHolder->size().width(),
                 _graphicsViewHolder->size().height());
  ...
  if (_aShowGrid->isChecked()) {
    ...
    for (float i = 0.0f; i * stepV <= h + stepV; i += 5.0f) {
      ...
      if (!_aGraphicsView->isChecked()) {
        if (painter) {
          painter->drawLine(0, stepV * i + borderVer + 0.5f, borderHor,
                            stepV * i + borderVer + 0.5f);
          painter->save();
          painter->setPen(pen);
          painter->drawLine(borderHor, stepV * i + borderVer + 0.5f,
                            w + borderHor, stepV * i + borderVer + 0.5f);
          painter->restore();
          painter->drawLine(w + borderHor, stepV * i + borderVer + 0.5f,
                            w + borderHor * 2, stepV * i + borderVer + 0.5f);
        }
      } else {
        hGridLines.append(
            new QGraphicsLineItem(0, stepV * i + borderVer, borderHor,
                                  stepV * i + borderVer, _sceneRoot));
        hGridLines.append(new QGraphicsLineItem(
            borderHor, stepV * i + borderVer, w + borderHor,
            stepV * i + borderVer, _sceneRoot));
        hGridLines.last()->setPen(pen);
        hGridLines.append(new QGraphicsLineItem(
            w + borderHor, stepV * i + borderVer, w + borderHor * 2,
            stepV * i + borderVer, _sceneRoot));
      }
    }
    for (float i = 0; i * stepH < w + stepH; i += 5.0f) {
      ...
      if (!_aGraphicsView->isChecked()) {
        if (painter) {
          painter->drawLine(stepH * i + borderHor + 0.5f, 0,
                            stepH * i + borderHor + 0.5f, borderVer);
          painter->save();
          painter->setPen(pen);
          painter->drawLine(stepH * i + borderHor + 0.5f, borderVer,
                            stepH * i + borderHor + 0.5f, h + borderVer);
          painter->restore();
          painter->drawLine(stepH * i + borderHor + 0.5f, h + borderVer,
                            stepH * i + borderHor + 0.5f, h + borderVer * 2);
        }
      } else {
        vGridLines.append(new QGraphicsLineItem(stepH * i + borderHor, 0,
                                                stepH * i + borderHor,
                                                borderVer, _sceneRoot));
        vGridLines.append(new QGraphicsLineItem(
            stepH * i + borderHor, borderVer, stepH * i + borderHor,
            h + borderVer, _sceneRoot));
        vGridLines.last()->setPen(pen);
        vGridLines.append(new QGraphicsLineItem(
            stepH * i + borderHor, h + borderVer, stepH * i + borderHor,
            h + borderVer * 2, _sceneRoot));
      }
    }
  }
  ...
  for (QList<UPlotCurve*>::iterator i = _curves.begin(); i != _curves.end();
    ...
    if ((*i)->isVisible()) {
      float xDir = 1.0f;
      float yDir = -1.0f;
      (*i)->update(scaleX, scaleY,
                   xDir < 0 ? axis[1] + borderHor / scaleX
                            : -(axis[0] - borderHor / scaleX),
                   yDir < 0 ? axis[3] + borderVer / scaleY
                            : -(axis[2] - borderVer / scaleY),
                   xDir, yDir, _aKeepAllData->isChecked());
      if (painter) {
        (*i)->draw(painter);
      }
    }
...
void UPlot::updateAxis(const UPlotCurve* curve) {
  if (curve && curve->isVisible() && curve->itemsSize() &&
      curve->isMinMaxValid()) {
    const QVector<float>& minMax = curve->getMinMax();
    // ULOGGER_DEBUG("x1=%f, x2=%f, y1=%f, y2=%f", minMax[0], minMax[1],
    // minMax[2], minMax[3]);
    if (minMax.size() != 4) {
...
void UPlot::contextMenuEvent(QContextMenuEvent* event) {
  ...
  if (!action) {
  ...
  } else if (action == _aLimit0 || action == _aLimit10 || action == _aLimit50 ||
             action == _aLimit100 || action == _aLimit500 ||
             action == _aLimit1000 || action == _aLimitCustom) {
    this->setMaxVisibleItems(action->text().toInt());
  } else if (action == _aAddVerticalLine || action == _aAddHorizontalLine) {
    bool ok;
    QString text =
        QInputDialog::getText(this, action->text(), tr("New line name :"),
                              QLineEdit::Normal, "", &ok);
    while (ok && text.isEmpty()) {
      QMessageBox::warning(
          this, action->text(),
          tr("The name is not valid or it is already used in this plot."));
      text = QInputDialog::getText(this, action->text(), tr("New line name :"),
                                   QLineEdit::Normal, "", &ok);
    }
    if (ok) {
      ...
      double value = QInputDialog::getDouble(
          ...
          (min + max) / 2, -2147483647, 2147483647, 4, &ok);
      if (ok) {
    ...
    text = QInputDialog::getText(this, _aChangeTitle->text(), tr("Title :"),
    ...
    QString text = QInputDialog::getText(
        this, _aChangeXLabel->text(), tr("X axis label :"), QLineEdit::Normal,
        _xLabel->text(), &ok);
    if (ok) {
      this->setXLabel(text);
    }
    ...
    bool ok;
    QString text = QInputDialog::getText(
        this, _aChangeYLabel->text(), tr("Y axis label :"), QLineEdit::Normal,
        _yLabel->text(), &ok);
    if (ok) {
      this->setYLabel(text, _yLabel->orientation());
    }
  } else if (action == _aYLabelVertical) {
    this->setYLabel(_yLabel->text(), _aYLabelVertical->isChecked()
                                         ? Qt::Vertical
                                         : Qt::Horizontal);
    ...
    text = QFileDialog::getSaveFileName(
        this, tr("Save figure to ..."),
        (QDir::homePath() + "/") + this->title() + ".png",
        "*.png *.xpm *.jpg *.pdf *.svg");
#else
    text = QFileDialog::getSaveFileName(
        this, tr("Save figure to ..."),
...
void UPlot::setWorkingDirectory(const QString& workingDirectory) {
  if (QDir(_workingDirectory).exists()) {
    ...
    ULOGGER_ERROR("The directory \"%s\" doesn't exist",
...
void UPlot::captureScreen() {
  ...
  targetDir += "/";
  QString name =
      (QDateTime::currentDateTime().toString("yyMMddhhmmsszzz") + ".") +
...
void UPlot::selectScreenCaptureFormat() {
  ...
  bool ok;
  QString item = QInputDialog::getItem(this, tr("Select format"), tr("Format:"),
                                       items, 0, false, &ok);
  if (ok && !item.isEmpty()) {
...
// for convenience...
UPlotCurveThreshold* UPlot::addThreshold(const QString& name, float value,
                                         Qt::Orientation orientation) {
  UPlotCurveThreshold* curve =
      new UPlotCurveThreshold(name, value, orientation, this);
  QPen pen = curve->pen();
...
void UPlot::addItem(QGraphicsItem* item) { item->setParentItem(_sceneRoot); }
...
void UPlot::keepAllData(bool kept) { _aKeepAllData->setChecked(kept); }
...
QRectF UPlot::sceneRect() const { return _view->sceneRect(); }

void UPlot::removeCurves() {
  QList<UPlotCurve*> tmp = _curves;
  for (QList<UPlotCurve*>::iterator iter = tmp.begin(); iter != tmp.end();
       ++iter) {
...
void UPlot::removeCurve(const UPlotCurve* curve) {
  QList<UPlotCurve*>::iterator iter =
      qFind(_curves.begin(), _curves.end(), curve);
#if PRINT_DEBUG
  ULOGGER_DEBUG("Plot=\"%s\" removing curve=\"%s\"",
                this->objectName().toStdString().c_str(),
                curve ? curve->name().toStdString().c_str() : "");
#endif
  if (iter != _curves.end()) {
    UPlotCurve* c = *iter;
    c->detach(this);
    ...
    if (!qobject_cast<UPlotCurveThreshold*>(c)) {
      ...
      for (int i = _curves.size() - 1; i >= 0; --i) {
        if (!qobject_cast<UPlotCurveThreshold*>(_curves.at(i))) {
          connect(_curves.at(i), SIGNAL(dataChanged(const UPlotCurve*)), this,
                  SLOT(updateAxis()));
          break;
        }
      }
