...
class MissionNode {
 public:
  ...
  void initMissionNode() {
    ...
    // 判断是否抵达导航地点的sub
    status_sub = nh.subscribe<move_base_msgs::MoveBaseActionResult>(
        "/move_base/result", 10, &MissionNode::status_cb, this);
    ar_sub = nh.subscribe<ar_track_alvar_msgs::AlvarMarkers>(
        "/ar_pose_marker", 10, &MissionNode::tag_cb, this);
    pos_sub = nh.subscribe<geometry_msgs::PoseStamped>(
        "/abot/pose", 1, &MissionNode::pos_cb, this);
    ...
    printf_param();
    int check_flag;
    cout << "Please check the parameter and setting，1 for go on， else for "
            "quit: "
         << endl;
    cin >> check_flag;
    // 执行循环
    exec_timer =
        nh.createTimer(ros::Duration(0.05), &MissionNode::execCallback, this);
 ...
 private:
  ...
  void pos_cb(const geometry_msgs::PoseStamped::ConstPtr &msg) {
    ...
    yaw_cur = atan2(2.0 * (quat[3] * quat[0] + quat[1] * quat[2]),
  ...
  void printf_result() {
    ...
    // cout << "pos_target: [X Z] : " << " " << offset_x  << " [m] " << offset_z
    // <<" [m] "<<endl;
