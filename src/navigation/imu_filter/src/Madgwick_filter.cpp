...
void filter_function(const sensor_msgs::Imu &msg) {
  ...
  // change Gyroscope units to radians/second, accelerometer units are
  // irrelevant as the vector is normalised.
...
int main(int argc, char **argv) {
  ...
  ros::NodeHandle n;
  ros::Publisher pub1 =
      n.advertise<geometry_msgs::QuaternionStamped>("quaternion", 1);
  ...
  if (!n.getParam("sampleFreq", sampleFreq)) sampleFreq = 400.0;
  ...
  while (ros::ok()) {
    ...
    q_trans.transform.rotation = tf::createQuaternionMsgFromRollPitchYaw(
        v.vector.z, v.vector.y, v.vector.x);
...
// reference: An efficient orientation filter for inertial and inertial/magnetic
// sensor arrays. (This thesis's appendix has more detailed code and
// illutration) reference:
...
void MadgwickAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay,
  ...
  float qDot1, qDot2, qDot3,
      qDot4;  // quaternion derrivative from gyroscopes elements
  float _2q0, _2q1, _2q2, _2q3, _4q0, _4q1, _4q2, _8q1, _8q2, q0q0, q1q1, q2q2,
      q3q3;  // Axulirary variables to avoid reapeated calcualtions
