...
AbotIMU::AbotIMU(ros::NodeHandle nh, ros::NodeHandle pnh)
  ...
  pnh_.param<bool>("imu/use_accelerometer", use_accelerometer_,
                   use_accelerometer_);
  pnh_.param<bool>("imu/use_gyroscope", use_gyroscope_, use_gyroscope_);
  pnh_.param<bool>("imu/use_magnetometer", use_magnetometer_,
                   use_magnetometer_);
  pnh_.param<bool>("imu/perform_calibration", perform_calibration_,
                   perform_calibration_);

  raw_sub_ = nh_.subscribe("raw_imu", 5, &AbotIMU::rawCallback, this);

  imu_cal_srv_ = nh_.advertiseService("imu/calibrate_imu",
                                      &AbotIMU::calibrateCallback, this);
  ...
  if (use_accelerometer_ || use_gyroscope_) {
    ...
    AbotIMU::fillRowMajor(linear_acc_covar_, linear_acc_stdev_);

    pnh_.param<double>("imu/angular_vel_stdev", angular_vel_stdev_, 0.0);
    AbotIMU::fillRowMajor(angular_vel_covar_, angular_vel_stdev_);
  }
  ...
  if (use_magnetometer_) {
    ...
    if (use_mag_msg_) {
      ...
      pnh_.param<double>("imu/magnetic_field_stdev", magnetic_field_stdev_,
                         0.0);
      AbotIMU::fillRowMajor(magnetic_field_covar_, magnetic_field_stdev_);
    } else {
...
void AbotIMU::rawCallback(const abot_imu::RawImuConstPtr& raw_msg) {
  ...
  if (perform_calibration_ || !is_calibrated_) {
    ROS_WARN_ONCE(
        "Calibrating accelerometer and gyroscope, make sure robot is "
    ...
    if (taken_samples < calibration_samples_) {
      ...
      acceleration_bias_["z"] =
          acceleration_bias_["z"] / calibration_samples_ + GRAVITY;
      ...
      ROS_INFO("Accelerometer: x: %f, y: %f, z: %f", acceleration_bias_["x"],
               acceleration_bias_["y"], acceleration_bias_["z"]);
      ROS_INFO("Gyroscope: x: %f, y: %f, z: %f", gyroscope_bias_["x"],
               gyroscope_bias_["y"], gyroscope_bias_["z"]);
    ...
    if (use_accelerometer_ || use_gyroscope_) {
      ...
      imu_msg->angular_velocity.x =
          raw_msg->raw_angular_velocity.x - gyroscope_bias_["x"];
      imu_msg->angular_velocity.y =
          raw_msg->raw_angular_velocity.y - gyroscope_bias_["y"];
      imu_msg->angular_velocity.z =
          raw_msg->raw_angular_velocity.z - gyroscope_bias_["z"];
      imu_msg->orientation_covariance = angular_vel_covar_;

      imu_msg->linear_acceleration.x =
          raw_msg->raw_linear_acceleration.x - acceleration_bias_["x"];
      imu_msg->linear_acceleration.y =
          raw_msg->raw_linear_acceleration.y - acceleration_bias_["y"];
      imu_msg->linear_acceleration.z =
          raw_msg->raw_linear_acceleration.z - acceleration_bias_["z"];
    ...
    if (use_magnetometer_) {
      if (use_mag_msg_) {
        sensor_msgs::MagneticFieldPtr mag_msg =
            boost::make_shared<sensor_msgs::MagneticField>();
        mag_msg->header = raw_msg->header;

        mag_msg->magnetic_field.x =
            (raw_msg->raw_magnetic_field.x * MILIGAUSS_TO_TESLA_SCALE -
             (mag_x_max_ - mag_x_min_) / 2 - mag_x_min_);
        mag_msg->magnetic_field.y =
            (raw_msg->raw_magnetic_field.y * MILIGAUSS_TO_TESLA_SCALE -
             (mag_y_max_ - mag_y_min_) / 2 - mag_y_min_);
        mag_msg->magnetic_field.z =
            (raw_msg->raw_magnetic_field.z * MILIGAUSS_TO_TESLA_SCALE -
             (mag_z_max_ - mag_z_min_) / 2 - mag_z_min_);
        mag_msg->magnetic_field_covariance = magnetic_field_covar_;

        mag_pub_.publish(mag_msg);
      } else {
        geometry_msgs::Vector3StampedPtr mag_msg =
            boost::make_shared<geometry_msgs::Vector3Stamped>();
        ;
        mag_msg->header = raw_msg->header;

        mag_msg->vector.x = (raw_msg->raw_magnetic_field.x -
                             (mag_x_max_ - mag_x_min_) / 2 - mag_x_min_) *
                            MILIGAUSS_TO_TESLA_SCALE;
        mag_msg->vector.y = (raw_msg->raw_magnetic_field.y -
                             (mag_y_max_ - mag_y_min_) / 2 - mag_y_min_) *
                            MILIGAUSS_TO_TESLA_SCALE;
        mag_msg->vector.z = (raw_msg->raw_magnetic_field.z -
                             (mag_z_max_ - mag_z_min_) / 2 - mag_z_min_) *
                            MILIGAUSS_TO_TESLA_SCALE;
...
bool AbotIMU::calibrateCallback(std_srvs::Empty::Request& request,
                                std_srvs::Empty::Response& response) {
  ROS_WARN(
      "Calibrating accelerometer and gyroscope, make sure robot is stationary "
      "and level.");
  perform_calibration_ = true;
  return true;
}

void AbotIMU::fillRowMajor(boost::array<double, 9>& covar, double stdev) {
