#include "shoot_cmd/control_center.h"
#include <cmath>
...
void ControlCenter::initROSModule() {
  ...
  status_sub = nn.subscribe<actionlib_msgs::GoalStatusArray>(
      "/move_base/status", 10, &ControlCenter::status_cb, this);
  pos_sub = nn.subscribe<tf2_msgs::TFMessage>("/tf", 1000,
                                              &ControlCenter::pos_cb, this);
  voice_sub = nn.subscribe<std_msgs::String>("/snowman/ask", 100,
                                             &ControlCenter::voice_cb, this);
  ar_sub = nn.subscribe<geometry_msgs::Point>("/ar_pose_marker", 10,
                                              &ControlCenter::ar_cb, this);

  cmd_pub = nn.advertise<geometry_msgs::Twist>("/cmd_vel", 10);
  goal_pub =
      nn.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal", 10);
  exec_timer_ =
      nn.createTimer(ros::Duration(0.05), &ControlCenter::execCallback, this);
}
...
Eigen::Quaterniond ControlCenter::quaternion_from_euler(float roll, float pitch,
                                                        float yaw) {
  Eigen::Quaterniond q;
...
void ControlCenter::execCallback(const ros::TimerEvent &e) {
  ...
  if (!pub_A) {
    ROS_INFO("Moving to A point at X: %f, Y:%f, Yaw: %f......", A[0], A[1],
             A_yaw);
  ...
  if (!pub_B) {
    ROS_INFO("Moving to B point at X: %f, Y:%f, Yaw: %f......", B[0], B[1],
             B_yaw);
  ...
  if (!pub_C) {
    ROS_INFO("Moving to C point at X: %f, Y:%f, Yaw: %f......", C[0], C[1],
             C_yaw);
  ...
  if (!pub_D) {
    ROS_INFO("Moving to D point at X: %f, Y:%f, Yaw: %f......", D[0], D[1],
             D_yaw);
