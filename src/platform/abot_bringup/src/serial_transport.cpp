...
void Serial_transport::start_a_read() {
  ...
  port_->async_read_some(
      boost::asio::buffer(temp_read_buf_),
      boost::bind(&Serial_transport::readHandler, this,
                  boost::asio::placeholders::error,
                  boost::asio::placeholders::bytes_transferred));
}

void Serial_transport::readHandler(const boost::system::error_code &ec,
  ...
  Buffer data(temp_read_buf_.begin(),
              temp_read_buf_.begin() + bytesTransferred);
...
void Serial_transport::start_a_write() {
  ...
  if (!write_buffer_.empty()) {
    boost::asio::async_write(*port_,
                             ...
                             boost::bind(&Serial_transport::writeHandler, this,
                                         boost::asio::placeholders::error));
    write_buffer_.pop();
  }
}
...
bool Serial_transport::init() {
  try {
    port_ = boost::make_shared<boost::asio::serial_port>(boost::ref(*ios_),
                                                         params_.serialPort);
    port_->set_option(boost::asio::serial_port::baud_rate(params_.baudRate));
    port_->set_option(boost::asio::serial_port::flow_control(
        (boost::asio::serial_port::flow_control::type)params_.flowControl));
    port_->set_option(boost::asio::serial_port::parity(
        (boost::asio::serial_port::parity::type)params_.parity));
    port_->set_option(boost::asio::serial_port::stop_bits(
        (boost::asio::serial_port::stop_bits::type)params_.stopBits));
  ...
  timer_ = boost::make_shared<boost::asio::deadline_timer>(
      boost::ref(*ios_), boost::posix_time::seconds(10));
