...
struct Robot_parameter {
  union {
    char buff[64];
    struct {
      unsigned short wheel_diameter = 97;  // 轮子直径  mm
      unsigned short wheel_track =
          225;  // 差分：轮距， 三全向轮：直径，四全向：前后轮距+左右轮距 mm
      unsigned short encoder_resolution = 3960;  // 编码器分辨率
      unsigned char do_pid_interval = 10;        // pid间隔 (ms)
      unsigned short kp = 320;
      unsigned short ki = 2700;
      unsigned short kd = 0;
      unsigned short ko = 10;  // pid参数比例
      unsigned short cmd_last_time =
          250;  // 命令持久时间ms 超过该时间会自动停止运动
      unsigned short max_v_liner_x = 50;
      unsigned short max_v_liner_y = 50;
      unsigned short max_v_angular_z = 250;
      unsigned char imu_type = 69;
    };
  };
};
