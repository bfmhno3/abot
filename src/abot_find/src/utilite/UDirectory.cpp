...
UDirectory::UDirectory(const UDirectory& dir) { *this = dir; }

UDirectory& UDirectory::operator=(const UDirectory& dir) {
  extensions_ = dir.extensions_;
  path_ = dir.path_;
  fileNames_ = dir.fileNames_;
  for (iFileName_ = fileNames_.begin(); iFileName_ != fileNames_.end();
       ++iFileName_) {
...
void UDirectory::update() {
  if (exists(path_)) {
    ...
    while (iter != fileNames_.end()) {
      valid = false;
      if (extensions_.size() == 0 && iter->compare(".") != 0 &&
          iter->compare("..") != 0) {
        valid = true;
      }
    ...
    if (!lastName.empty()) {
      bool found = false;
      for (std::list<std::string>::iterator iter = fileNames_.begin();
           iter != fileNames_.end(); ++iter) {
...
bool UDirectory::isValid() { return exists(path_); }
...
void UDirectory::rewind() { iFileName_ = fileNames_.begin(); }
...
#endif
  r = (dwAttrib != INVALID_FILE_ATTRIBUTES &&
