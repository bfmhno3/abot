...
/**
 * UPlotItem is a QGraphicsEllipseItem and can be inherited to do custom
 * behaviors on an hoverEnterEvent() for example.
 */
...
class UPlotCurve : public QObject {
 ...
 public:
   ...
   * Constructor 2
   */
  UPlotCurve(const QString& name, const QVector<UPlotItem*> data,
             QObject* parent = 0);
  /**
   * Constructor 3
   */
  UPlotCurve(const QString& name, const QVector<float>& x,
  ...
  void getData(QVector<float>& x,
               QVector<float>& y) const;  // only call in Qt MainThread
 ...
 public Q_SLOTS:
  ...
  /**
   *
   * Add a single value y, x is auto-incremented by the increment set with
   * setXIncrement().
   * @see setXStart()
   */
  ...
  /**
   *
   * Add multiple values y, x is auto-incremented by the increment set with
   * setXIncrement().
   * @see setXStart()
   */
 ...
 protected:
  ...
  virtual void update(float scaleX, float scaleY, float offsetX, float offsetY,
                      float xDir, float yDir, bool allDataKept);
...
/**
 * A special UPlotCurve that shows as a line at the specified value, spanning
 * all the UPlot.
 */
class UPlotCurveThreshold : public UPlotCurve {
 ...
 public:
   ...
   * Constructor.
   */
  UPlotCurveThreshold(const QString& name, float thesholdValue,
...
class UPlotAxis : public QWidget {
 public:
   ...
   * Constructor.
   */
  UPlotAxis(Qt::Orientation orientation = Qt::Horizontal, float min = 0,
 ...
 public:
   ...
   */
  void setReversed(
      bool reversed);  // Vertical :bottom->up, horizontal :right->left
...
class UOrientableLabel : public QLabel {
 ...
 public:
   ...
   * Constructor.
   */
  UOrientableLabel(const QString& text,
                   Qt::Orientation orientation = Qt::Horizontal,
                   QWidget* parent = 0);
...
class UPlot : public QWidget {
 ...
 public:
  ...
  /**
   * Add a curve. The returned curve doesn't need to be deallocated (UPlot keeps
   * the ownership).
   */
  UPlotCurve* addCurve(const QString& curveName,
                       const QColor& color = QColor());
  /**
   * Add a curve. Ownership is transferred to UPlot if
   ...
   */
  UPlotCurveThreshold* addThreshold(
