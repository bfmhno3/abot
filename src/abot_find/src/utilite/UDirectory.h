...
class UDirectory {
 ...
 public:
  /**
   * Create a UDirectory object with path initialized to an existing "path" and
   * with filter "extensions".
   * @param path the path to an existing directory
   * @param extensions filter to get only file names with the extensions
   * specified, format is a list of extensions separated by a space: "jpg bmp"
   * get only file names finishing by jpg or bmp.
   */
